...
impl ASTKind {
  pub fn verify_term<'a>(&self, node: &'a (ASTNode<'a>, Location<'a>)) {
    match self {
      ASTKind::Tuple => if !node.0.is_valid_tuple_term() {
        CompError::new(
          11,
      ...
      ASTKind::ArgTuple => if !node.0.is_valid_argtuple_term() {
        CompError::new(
          12,
      ...
      ASTKind::Block => if !node.0.is_valid_block_term() {
        CompError::new(
          13,
      ...
      ASTKind::File => if !node.0.is_valid_file_term() {
        CompError::new(
          14,
...
fn expect_next_instruction<'a>(tree: Rc<TokenTree<'a>>, offset: &mut usize) {
  if *offset == 0 || tree.tokens.len() == *offset {return}
  let old_loc = &tree.tokens[*offset - 1].1;
  let new_loc = &tree.tokens[*offset].1;

  if old_loc.line < new_loc.line {return}
