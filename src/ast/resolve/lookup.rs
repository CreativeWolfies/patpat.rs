...
*/
pub fn lookup_pattern<'a, 'b>(
    name: String,
    loc: Location<'b>,
    patterns: &'b Vec<RPatRef<'a>>,
    parent: RASTWeak<'a>,
) -> RPatRef<'a> {
    for pat in patterns {
...
*/
pub fn lookup_struct<'a, 'b>(
    name: TypeName,
    loc: Location<'b>,
    structs: &'b Vec<RStructRef<'a>>,
    parent: RASTWeak<'a>,
) -> RStructRef<'a> {
    for st in structs {
