...
impl<'a> RStruct<'a> {
    ...
    pub fn add_interpretation(&mut self, to: RStructWeak<'a>, body: AST<'a>, loc: Location<'a>, parent: RASTWeak<'a>) {
        let init = Rc::new(RefCell::new(RAST::new(parent)));
        init.borrow_mut().variables.push(Rc::new(RefCell::new(RSymbol::new(String::from("from")))));
        init.borrow_mut().variables.push(Rc::new(RefCell::new(RSymbol::new(String::from("to")))));
